		...
		<table>
			<thead>
				<tr>
					...
					<th class="head-price"><span><?php esc_attr_e( 'Warehouse Code', 'woocommerce-delivery-notes' ); ?></span></th>
				</tr>
			</thead>
					...
					<?php foreach ( $order->get_items() as $item_id => $item ) : ?>
						...
						<tr>
							...
							<td class="product-price">
								<span><?php
								try {
									if ($product->is_type('simple'))  //for single products
										{
											echo get_post_meta($product_id, 'my_custom_code', true);
										}
									else {  //for product variations
                    //Debugging
										//var_dump(get_post_meta($item['variation_id']));   
										//$variation_attributes = get_post_meta($item['variation_id']);
										echo get_post_meta($item['variation_id'], 'my_custom_code', true);
									}
								}
								catch (Exception $e) {
